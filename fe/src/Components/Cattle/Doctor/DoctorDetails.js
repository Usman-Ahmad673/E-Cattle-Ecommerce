    ...
    import { assignCattleToDoctor, getDoctorDetails } from '../../../actions/doctorActions';
    import { CircularProgress, Container, Grid, Typography, Paper, Card, CardMedia, Table, TableHead, TableRow, TableCell, TableContainer, TableBody, IconButton, InputBase, Button, ImageList, ImageListItem } from '@mui/material';
...
import { ASSIGN_CATTLE_RESET } from '../../../constants/doctorConstants';
    ...
    const { id } = useParams();
    const { doctor, loading, success } = useSelector((state) => state.doctorDetail);
    console.log(doctor);
    console.log(doctor?.cattle);
    
    const { cattles, loading : cattleLoading , error } = useSelector((state) => state.cattle);
    console.log(cattles);
    


    const { success : assignCattleSuccess, message , error: assignCattleError } = useSelector((state) => state.assignCattle);
        ...
        {loading && !doctor ? (
            ...
            <Container maxWidth='xl' sx={{ marginTop: 8}}>
                ...
                <Grid item xs={12} md={6} color='green'>
                    <Typography style={styles} variant="h4">Doctor Details</Typography>
                    <hr/>
                ...
                <Grid item xs={12} md={3}>
                    <Typography variant='h4' style={styles}>Cattle Details</Typography>
                    <hr />
                    <TableContainer component={Paper}>
                    <Table>
                        <TableHead>
                        <TableRow>
                            <TableCell sx={{ fontWeight: 'bold', fontSize: '0.8rem', color: 'gray' }}>Cattle ID</TableCell>
                            <TableCell sx={{ fontWeight: 'bold', fontSize: '0.8rem', color: 'gray' }}>Medicines</TableCell>
                            <TableCell sx={{ fontWeight: 'bold', fontSize: '0.8rem', color: 'gray' }}>Diseases</TableCell>
                            <TableCell sx={{ fontWeight: 'bold', fontSize: '0.8rem', color: 'gray' }}>Injections</TableCell>
                            {/* <TableCell sx={{ fontWeight: 'bold', fontSize: '0.8rem', color: 'gray' }}>Date Created</TableCell> */}
                        </TableRow>
                        </TableHead>
                        <TableBody>
                            {/* <TableCell>{doctor.cattle[0].medicines[0].name}</TableCell> */}
                        {doctor?.cattle.length > 0 ? doctor.cattle.map((cattle , index) => (
                        <TableRow>
                            <TableCell key={index}>{cattle?._id}</TableCell>
                            
                                <TableCell>
                                        <span>{cattle.medicines?.name === '' ? 'N/A' : cattle.medicines?.name}</span>
                                </TableCell>
                            <TableCell>
                                        <span>{cattle.diseases?.name === '' ? 'N/A' : cattle.diseases?.name}</span>
                                </TableCell>
                                <TableCell>
                                        <span>{cattle.injections?.name === '' ? 'N/A' : cattle.injections?.name}</span>
                                </TableCell>
                        </TableRow>
                        )):
                        <TableRow>
                            <TableCell align='center' colSpan={5}>No Cattle Record Against {doctor?.name}</TableCell>
                        </TableRow>
                        }
                        </TableBody>
                    </Table>
                ...
                <Grid item container xs={12} sm={12} md={12} rowSpacing={2} columnSpacing={2}>
                    <Typography sx={{backgroundColor: 'rgba(0 , 200 , 0 , 0.7)' , color:'red' , padding:'0.6rem' , display:'flex' , justifyContent:'center' , alignItems:'center'}} variant="h4" width='100%' margin='2rem auto' boxShadow='3px 5px 20px 10px rgba(50 , 200 , 0 , 0.5)'>Cattle List</Typography>
                    {cattles.map((cattle) => (
                        ...
                        {/* <Container maxWidth='xl' key={cattle._id}> */}
                            ...
                            {/* <div sx={{display: 'flex' , justifyContent: 'space-around' , alignItems: 'center' }}> */}
                                ...
                                <Typography variant='h6' color='blue'>{cattle._id}</Typography>
                                <Button 
                                sx={{ 
